...
public class PlayerPositionAndDimension implements IPlayerPosAndDim
{
	...
	public void loadNBTData(CompoundTag nbtTag)
	{
		...
		//Non-UA stuff
		storedDimension = RegistryKey.of(Registry.DIMENSION, new Identifier(nbtTag.getString("NonUADimensionPath")));
		...
		{
			storedPositionNonUA = new Vec3d(nbtTag.getFloat("NonUA_X"), nbtTag.getFloat("NonUA_Y"), nbtTag.getFloat("NonUA_Z"));
		}
		...
		{
			storePositionUA = new Vec3d(nbtTag.getFloat("UA_X"), nbtTag.getFloat("UA_Y"), nbtTag.getFloat("UA_Z"));
		}
