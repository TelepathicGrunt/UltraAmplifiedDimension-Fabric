...
public class BiomeGroup implements Comparable<BiomeGroup> {
    public static final Codec<BiomeGroup> CODEC = RecordCodecBuilder.create((instance) -> instance.group(
            RegistryElementCodec.of(Registry.BIOME_KEY, Biome.CODEC).fieldOf("main_biome").forGetter(biomeGroup -> biomeGroup.mainBiome),
            Codec.intRange(1, Integer.MAX_VALUE).fieldOf("weight").forGetter(biomeGroup -> biomeGroup.weight),
            RegistryElementCodec.of(Registry.BIOME_KEY, Biome.CODEC).optionalFieldOf("shore_biome").forGetter(biomeGroup -> biomeGroup.shoreBiome),
            RegistryElementCodec.of(Registry.BIOME_KEY, Biome.CODEC).optionalFieldOf("border_biome").forGetter(biomeGroup -> biomeGroup.borderBiome),
            RegistryElementCodec.of(Registry.BIOME_KEY, Biome.CODEC).optionalFieldOf("sub_biome").forGetter(biomeGroup -> biomeGroup.subBiome),
            RegistryElementCodec.of(Registry.BIOME_KEY, Biome.CODEC).optionalFieldOf("mutated_biome").forGetter(biomeGroup -> biomeGroup.mutatedBiome),
            RegistryElementCodec.of(Registry.BIOME_KEY, Biome.CODEC).optionalFieldOf("mutated_sub_biome").forGetter(biomeGroup -> biomeGroup.mutatedSubBiome),
            RegistryElementCodec.of(Registry.BIOME_KEY, Biome.CODEC).optionalFieldOf("mutated_border_biome").forGetter(biomeGroup -> biomeGroup.mutatedBorderBiome)
