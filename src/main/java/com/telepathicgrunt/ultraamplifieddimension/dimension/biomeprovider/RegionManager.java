...
public class RegionManager {
    ...
    private MutablePair<List<BiomeGroup>, Integer> prepareListAndPopulateBiomeRelationMaps(MutablePair<List<BiomeGroup>, Integer> regionGroup){
        MutablePair<List<BiomeGroup>, Integer> nonImmutableListMutablePair = new MutablePair<>(new ArrayList<>(regionGroup.getLeft()), regionGroup.getRight());
        ...
        nonImmutableListMutablePair.getLeft().sort(BiomeGroup::compareTo);

        for(BiomeGroup group : nonImmutableListMutablePair.getLeft()){
        ...
        return nonImmutableListMutablePair;
    }
     ...
     */
    public BiomeGroup getRandomBiomeGroup(MutablePair<List<BiomeGroup>, Integer> regionGroup, LayerRandomnessSource context){
        // Now choose a random item.
        int index = 0;
        for (double randomWeightPicked = context.nextInt(regionGroup.getRight()); index < regionGroup.getLeft().size() - 1; ++index) {
    ...
    public List<Biome> importAllModdedBiomes(Registry<Biome> biomeRegistry, HashSet<Identifier> blacklistedModdedBiomes){
        ...
        for(Map.Entry<RegistryKey<Biome>, Biome> entry : biomeRegistry.getEntries()){
            ...
            if(entry.getKey().getValue().getNamespace().equals("minecraft") ||
                    entry.getKey().getValue().getNamespace().equals(UltraAmplifiedDimension.MODID) ||
                    blacklistedModdedBiomes.contains(entry.getKey().getValue()))
            {
                continue;
            }
