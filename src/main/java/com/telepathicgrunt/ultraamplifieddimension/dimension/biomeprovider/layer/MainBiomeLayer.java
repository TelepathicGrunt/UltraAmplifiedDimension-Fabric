...
public class MainBiomeLayer implements CastleWithPositionTransformer {
    ...
    public int apply(LayerRandomnessSource context, int north, int west, int south, int east, int regionID, int x, int z) {
        ...
        if(regionID == UADBiomeProvider.REGIONS.END.ordinal()){
            ...
            // It is /16.8 instead of /4.2 due to the two zoom layers after the base layer.
            double baseRegionNoiseCopy = (context.getNoiseSampler().sample(
                    (double)x / 16.8D,
                    (double)z / 16.8D,
                    0.0D,
                    0.0D,
        ...
        return dynamicRegistry.getRawId(selectedBiomeGroup.getMainBiome().get());
