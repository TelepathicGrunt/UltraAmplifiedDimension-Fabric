...
public class ShoreEdgeHillsAndMutatationsBiomeLayer implements CastleWithPositionTransformer {
    ...
    public int apply(LayerRandomnessSource context, int north, int west, int south, int east, int originalBiomeID, int x, int z) {
        Biome currentBiome = dynamicRegistry.get(originalBiomeID);
        Biome nbiome = dynamicRegistry.get(north);
        Biome wbiome = dynamicRegistry.get(west);
        Biome ebiome = dynamicRegistry.get(east);
        Biome sbiome = dynamicRegistry.get(south);
        Biome newBiome = null;
        ...
        if(newBiome != null){
            return dynamicRegistry.getRawId(newBiome);
        }

        // Creates the sub, mutated, and mutated sub biomes
        double subBiomeNoise = (context.getNoiseSampler().sample(
                (double)x / biomeSize,
                (double)z / biomeSize,
                2314.0D,
                0.0D,
        ...
        double mutatedNoise = (context.getNoiseSampler().sample(
            ...
            sbiome.getCategory() != currentBiome.getCategory())
        {
            ...
            if(mutatedNoise < mutatedThreshold) {
                ...
                if (mutatedBorderBiome != null) {
                    return dynamicRegistry.getRawId(mutatedBorderBiome);
                }
            }
        }
        ...
        // There's no sub border so we can do else if here safely
            ...
            if(mutatedNoise < mutatedThreshold) {
                ...
                if(mutatedSubBiome != null){
                    return dynamicRegistry.getRawId(mutatedSubBiome);
                }
            }
        }
