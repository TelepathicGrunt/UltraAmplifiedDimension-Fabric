...
public class IcebergWithoutAir extends Feature<SingleStateFeatureConfig> {

    public IcebergWithoutAir(Codec<SingleStateFeatureConfig> configFactory) {
        super(configFactory);
    }
    ...
    private void func_205174_a(int heightThickness, int belowCenterY, BlockPos position, WorldAccess world, boolean placeWater, double double1, BlockPos pos2,int int1, int int2)
    {
        ...
        for (int x = -radius; x < radius; ++x) {
            for (int z = -radius; z < radius; ++z) {
                ...
                if (d0 < 0.0D) {
                    blockpos$Mutable.set(position).move(x, belowCenterY, z);
                    blockState = world.getBlockState(blockpos$Mutable);
                    if (blockState.isIn(BlockTags.ICE) || blockState.isOf(Blocks.SNOW_BLOCK)) {
    ...
    private void removeSnowLayer(WorldAccess world, BlockPos.Mutable mutableBlockPos)
    {
        if (world.getBlockState(mutableBlockPos.move(Direction.UP)).isOf(Blocks.SNOW)) {
    ...
    private void func_205181_a(WorldAccess world, Random random, BlockPos position,int maxHeight, int xPos, int yPos, int zPos, int int1, int int2, boolean flag1, int int3, double double1, boolean flag2, BlockState defaultState)
    {
        double noise = flag1 ? this.func_205180_a(xPos, zPos, BlockPos.ORIGIN, int2, this.func_205176_a(yPos, maxHeight, int3), double1) : this.func_205177_a(xPos, zPos, BlockPos.ORIGIN, int1, random);
        if (noise < 0.0D) {
    ...
    private void func_205175_a(BlockPos position, WorldAccess world, Random random,int minHeight, int maxHeight, boolean flag1, boolean flag2, BlockState defaultState)
    {
        BlockState currentBlockState = world.getBlockState(position);

        if (currentBlockState.isAir() || currentBlockState.isOf(Blocks.SNOW_BLOCK) || currentBlockState.isIn(BlockTags.ICE) || !currentBlockState.getFluidState().isEmpty()) {
    ...
    private BlockState appropriateBlockForNeighbors(WorldAccess world, BlockPos position)
    {
        ...
        for (int x = -radius; x <= radius; ++x) {
            for (int z = -radius; z <= radius; ++z) {
                for (int y = 0; y <= height; ++y) {
                    ...
                    if (blockState.isIn(BlockTags.ICE) || blockState.isOf(Blocks.SNOW)) {
    ...
    public boolean isAirBelow(BlockView worldIn, BlockPos pos) {
