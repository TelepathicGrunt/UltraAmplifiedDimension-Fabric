...
public class NbtDungeon extends Feature<NbtDungeonConfig>{
    ...
    public boolean generate(FeatureContext<NbtDungeonConfig> context) {
        ...
        BlockPos fullLengths = new BlockPos(
                Math.abs(template.getSize().getX()),
                Math.abs(template.getSize().getY()),
                Math.abs(template.getSize().getZ()))
                .rotate(rotation);
        ...
        if (wallOpenings >= context.getConfig().minAirSpace) {
            ...
            template.place(context.getWorld(), mutable, mutable, placementsettings, context.getRandom(), Block.NO_REDRAW);
            ...
            List<Structure.StructureBlockInfo> list = placementsettings.getRandomBlockInfos(((TemplateAccessor)template).uad_getBlocks(), mutable).getAll();
