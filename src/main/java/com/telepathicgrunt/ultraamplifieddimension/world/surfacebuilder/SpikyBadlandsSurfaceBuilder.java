...
public class SpikyBadlandsSurfaceBuilder extends BadlandsSurfaceBuilder {
    public SpikyBadlandsSurfaceBuilder(Codec<TernarySurfaceConfig> codec) {
        super(codec);
    }
    ...
    public void generate(Random random, Chunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, TernarySurfaceConfig config) {
        ...
        double spikeNoise = Math.min(Math.abs(noise), this.heightCutoffNoise.sample(x * 0.20D, z * 0.20D, false) * 15.0D);
        if (spikeNoise > -2D) {
